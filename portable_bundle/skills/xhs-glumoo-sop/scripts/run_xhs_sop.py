...
def infer_strategy(unit_price: float | None, user_intent: str | None, content_type: str | None, sku_profile: dict | None = None) -> str:
...
def get_sku_profile(theme_cfg: dict | None, sku: int) -> dict:
    if theme_cfg:
...
def sanitize_copy_text(text: str, sku_profile: dict | None = None):
    sanitized = text
    hits = []
...
def maybe_write_strategy_snapshot(out_dir: Path, strategy: str, args, sku_profile: dict, sanitize_hits: list | None = None):
    snapshot = {
        "strategy": strategy,
