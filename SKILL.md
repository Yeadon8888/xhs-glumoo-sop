...
Text/image model config supported:
- Text: `XHS_TEXT_API_KEY`, `XHS_TEXT_BASE_URL`, `XHS_TEXT_MODEL`
- Image: `XHS_IMAGE_API_KEY`, `XHS_IMAGE_BASE_URL`, `XHS_IMAGE_MODEL`, `XHS_IMAGE_ASPECT_RATIO`
- For the locked daily workflow, image generation is pinned to the XHS/Yunwu variables above and does not rely on Google image fallbacks.

Recommended portable defaults for Yunwu:
- Text model: `models/gemini-3.1-flash-lite-preview`
...
- Image gen: `skills/xhs-glumoo-sop/scripts/generate_image_compat.py` (local Yunwu-first wrapper; only uses `XHS_IMAGE_API_KEY` + `XHS_IMAGE_BASE_URL`, no Google fallback)
...
Review-only send safety:
- The generated report's `## images` section is the Feishu-safe send list.
- These paths are staged into `/Users/Apple/.openclaw/media/outbox/feishu-longxia-dabao/...`
- The report keeps only `## debug_source_image_names_do_not_send` for debugging; it no longer exposes worker absolute paths.
- The runner now suppresses raw worker `MEDIA:/Users/Apple/.openclaw/workspace-worker-xhs/...` lines and emits safe `MEDIA:` lines for the staged outbox paths at the end.
- When sending to Feishu, use only the staged outbox paths or report `## images`; never send `## source_images` or raw worker log paths.
- Reports now avoid exposing worker absolute paths in debug sections; if you need to resend images later, resolve them from `## images`, not from `out/xhs_autorun/*.log`.
...
### Locked daily runner (recommended)

Use this when you want the Glumoo daily workflow to stay fixed instead of re-deciding paths/models each day:

```bash
XHS_TEXT_API_KEY="..." \
XHS_IMAGE_API_KEY="..." \
bash skills/xhs-glumoo-sop/scripts/run_xhs_sop_locked.sh
```

What this locked runner freezes:
- review-only daily run
- auto theme / auto sku / auto content type
- image generation pinned to `https://yunwu.ai`
- image model pinned to `gemini-3.1-flash-image-preview`
- text model default pinned to `models/gemini-3.1-flash-lite-preview`
- no Google image fallback path
